...
ifeq ($(USE_CUDA),1) 
	...
	BUILD_DIR_ROOT := build/$(HOSTNAME)/CUDA
endif
...
DEBUG_BUILD_DIR := $(BUILD_DIR_ROOT)/debug
...
UTIL_OPENCL_BUILD_DIR := $(OBJ_BUILD_DIR)/util/OpenCL
...
OPENCL_INCLUDE_DIR += $(CLBLAS_DIR)/include
OPENCL_LIB_DIR :=
ifneq ("$(wildcard $(OPENCL_DIR)/lib/x86_64)","")
	OPENCL_LIB_DIR += $(OPENCL_DIR)/lib/x86_64
	OPENCL_LIB_DIR += $(OPENCL_DIR)/lib/x86_64/sdk
	OPENCL_LIB_DIR += $(OPENCL_DIR)/lib64	
endif
OPENCL_LIB_DIR += $(OPENCL_DIR)/lib/x86
OPENCL_LIB_DIR += /usr/lib
OPENCL_LIB_DIR += $(CLBLAS_DIR)/lib64
...
ifeq ($(USE_OPENCL), 1)
	...
	LIBRARY_DIRS += $(OPENCL_LIB_DIR)
	LIBRARIES := OpenCL clBLAS boost_regex
endif
...
ALL_BUILD_DIRS := $(sort \
		...
		$(LAYER_BUILD_DIR) $(UTIL_BUILD_DIR) $(UTIL_OPENCL_BUILD_DIR) $(TOOL_BUILD_DIR) \
...
#	TEST_FILTER := --gtest_filter="*GPU*"
	...
	ifeq ("$(CAFFE_OPENCL_VERSION)", "0.1")
		COMMON_FLAGS += -DOPENCL_OPT_LEVEL=1
	else
		COMMON_FLAGS += -DOPENCL_OPT_LEVEL=0	
	endif
endif
...
$(BUILD_DIR)/.linked:
	echo "$(BUILD_DIR)";
	@ if [ ! -d "$(BUILD_DIR_ROOT)" ]; then mkdir -p $(BUILD_DIR_ROOT); fi
...
$(UTIL_OPENCL_BUILD_DIR)/%.o: src/$(PROJECT)/util/OpenCL/%.cpp $(HXX_SRCS) | $(UTIL_OPENCL_BUILD_DIR)
	$(CXX) $< $(CXXFLAGS) -c -o $@ 2> $@.$(WARNS_EXT) \
		|| (cat $@.$(WARNS_EXT); exit 1)
	@ cat $@.$(WARNS_EXT)
	@ echo
